...
enum PatchTransaction {
    ...
    static func restore(
        ...
        var roots: [String: URL] = [:]
        for bundleID in Set(journal.records.map(\.bundleID) + (journal.createdDirectories ?? []).map(\.bundleID)) {
            do {
                let root = PatchPathValidator.canonicalFileURL(try containerResolver(bundleID))
                roots[bundleID] = root
            } catch {
                // App sumiu / container mudou — segue best-effort nos que existirem
                continue
            }
        }

        // Sem containers: ainda marca restored pra nao ficar preso em ON
        if roots.isEmpty {
        ...
        do {
            try restoreRecords(
                ...
                roots: roots,
                requirePatchedDigest: false, // game pode ter sobrescrito o arquivo
            ...
            // Ultimo recurso: abandona o receipt pra desligar o toggle
            journal.status = .restored
            try? writeJournal(journal, to: receipt.journalURL)
            throw PatchPackageError.restoreFailed
        }
    ...
    /// Marca journal como restored sem tocar nos arquivos (força OFF na UI).
    static func abandonReceipt(
        _ receipt: PatchTransactionReceipt,
        fileManager: FileManager = .default
    ) {
        guard var journal = try? readJournal(receipt.journalURL) else { return }
        journal.status = .restored
        try? writeJournal(journal, to: receipt.journalURL)
    }
    ...
    private static func restoreRecords(
        ...
        for record in records {
            ...
            // Fingerprint so e checado em modo estrito (container UUID muda apos reinstall)
            if !bestEffort, containerFingerprint(root) != record.containerFingerprint {
                throw PatchPackageError.restoreFailed
            }
            ...
            do {
                ...
                try validateFileTarget(
                    ...
                    relativePath: record.relativePath,
                    containerRoot: root,
                    allowMissingParents: !requirePatchedDigest || bestEffort,
                    fileManager: fileManager
                )
            } catch {
                if bestEffort { continue }
            ...
            if requirePatchedDigest && !bestEffort {
        ...
        for directory in createdDirectories.reversed() {
            ...
            if !bestEffort, containerFingerprint(root) != directory.containerFingerprint {
                throw PatchPackageError.restoreFailed
            }
            do {
                let target = try PatchPathValidator.resolveContainedTargetURL(
    ...
    private static func atomicCopy(
        ...
        try handle.close()
        guard rename(staging.path, target.path) == 0 else {
            // Fallback quando rename falha (cross-device / perms)
            do {
                if fileManager.fileExists(atPath: target.path) {
                    try fileManager.removeItem(at: target)
                }
